...
5. **1 分钟以内素材默认允许通过拆段完成，不要求单次处理完整 60s。**
6. `<=60s` 默认拆分策略：
   - 高动态舞蹈 / 强动作控制：优先 `15s × N`；
   - 动作连续、模型端已验证稳定：可用 `20–30s × N`；
   - 分段边界必须与最终 BGM / Timeline 边界一致，不随意切在动作峰值中间。
7. 每个 Depth 分段独立产出 Raw / Temporal / Compare / report，并独立通过 QA；最终生成端再按段调用，不要求先把 Depth 重新拼成长视频。
8. 默认同时产出：
   - `depth_v2_raw_h264.mp4`：高细节原始深度；
   - `depth_v2_temporal_h264.mp4`：光流辅助时序稳定版；
   - `original_vs_depth_h264.mp4`：人工 QA 对比版；
   - `report.json`：真实规格。
9. Seedance 动作参考优先使用 Temporal 版；Raw 版用于细节对照和必要时 A/B。
10. 输入视频优先选择：单人、主体清楚、遮挡少、镜头稳定、动作连续、关键肢体可见。
11. 如果目标包含“脚步不滑行 / 重心落脚”验证，素材必须完整看到脚部和地面接触；半身舞不允许代替脚步验证。
