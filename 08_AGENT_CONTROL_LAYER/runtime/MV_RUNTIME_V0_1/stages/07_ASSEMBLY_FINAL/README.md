# Stage ASSEMBLY_FINAL

Status: NOT_STARTED

## Purpose

Assemble only accepted segments, verify continuity/audio/technical state and deliver final film.

## Current development lock

Detailed design is blocked until: VIDEO_QA SEALED.

This folder is currently a placeholder only.

Do not add deep Stage logic before the dependency is SEALED.

## Expected future delivery

Final video + final QA + human approval.

## Future build sequence

1. Contract Design
2. Test Plan
3. Isolated Test
4. Delivery Artifact
5. Review
6. PASS -> SEALED
7. Allow next Stage design

## Non-goal now

No detailed rules, prompts, Judge schema, or Skill implementation are locked in this placeholder.